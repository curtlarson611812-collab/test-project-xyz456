...
use speedbitcrack::utils::bias::{get_magic9_bias, MAGIC9_BIASES};

#[test]
fn test_gold_cluster_bias_patterns() {
    // Verify all GOLD cluster keys have identical bias patterns (universal zeros)
    assert!(
        !MAGIC9_BIASES.is_empty(),
        "MAGIC9_BIASES should be populated by build.rs"
    );
    ...
    if !MAGIC9_BIASES.is_empty() {
        ...
        println!(
            "✅ GOLD Cluster validation passed: All {} keys share identical bias patterns",
            MAGIC9_BIASES.len()
        );
    }
}
...
fn test_magic9_bias_getter() {
    ...
    assert_eq!(
        fallback_bias,
        (0, 0, 0, 0, 128),
        "Should return fallback bias for invalid index"
    );
}
...
fn test_gold_cluster_detection() {
    ...
    assert!(
        MAGIC9_BIASES.len() == 9,
        "Should have exactly 9 Magic 9 biases"
    );
}
