...
mod tests {
    ...
    fn test_small_range_solve() {
        ...
        config.target_x =
            Some("c6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee5".to_string());
        config.target_y =
            Some("1ae168fea63dc339a3c58419466ceaeef7f632653266d0e1236431a950cfe52a".to_string());
        ...
        match tokio_test::block_on(manager.run()) {
            ...
            Ok(None) => {
                ...
                println!(
                    "No solution found in test range - this is acceptable for integration testing"
                );
            }
            Err(e) => {
    ...
    fn test_performance_regression() {
        ...
        assert!(
            ...
            "Performance regression: took {:?}, expected < 30s",
            duration
        );
    }
    ...
    fn test_gpu_step_batch_integration() {
        ...
        let backend = match speedbitcrack::gpu::backend::CudaBackend::new() {
            ...
            Err(e) => {
                println!(
                    "CUDA backend not available for integration test: {} - skipping",
                    e
                );
                return;
            }
        ...
        for i in 0..batch_size {
            ...
            positions[i][0] = [
                0x79BE667E, 0xF9DCBBAC, 0x55A06295, 0xCE870B07, 0x29BFCDB2, 0xDCE28D95, 0x9F2815B1,
                0x6F81798,
            ]; // X coord
            positions[i][1] = [
        ...
        let (points, dists) = backend
            .precomp_table(vec![base_distance; 8], base_distance)
            .unwrap();
        ...
        println!(
            "GPU integration test completed successfully with {} traps found",
            traps.len()
        );
    }
    ...
    fn test_gpu_backend_availability() {
        #[cfg(feature = "cudarc")]
        {
            ...
            match speedbitcrack::gpu::backend::CudaBackend::new() {
                ...
                Err(e) => println!(
                    "CUDA backend not available: {} - this is expected on systems without CUDA",
                    e
                ),
            }
        }
        ...
        #[cfg(feature = "vulkan")]
        {
            ...
            match speedbitcrack::gpu::backend::VulkanBackend::new() {
                ...
                Err(e) => println!(
                    "Vulkan backend not available: {} - this is expected on systems without Vulkan",
                    e
                ),
            }
        }
    ...
    fn test_pollard_with_biases_primes() {
        ...
        let range = (
            speedbitcrack::math::bigint::BigInt256::zero(),
            speedbitcrack::math::bigint::BigInt256::from_u64(1000),
        ); // Small range
