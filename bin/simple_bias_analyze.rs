...
fn main() -> Result<()> {
    ...
    println!(
        "🎯 Computing REAL statistical biases for all {} pubkeys...",
        pubkeys.len()
    );
    ...
    let bias_scores: Vec<(String, f64, usize)> = pubkeys
        .par_iter()
        .enumerate()
        .map(|(_i, point)| {
            ...
            let mod81_residue = (x % BigInt256::from_u64(81)).to_u64() as usize;
            ...
            let mod27_deviation =
                (mod27_counts[mod27_residue] as f64 - mod27_expected) / mod27_expected;
            score += mod27_deviation.abs() * 0.25;

            // Mod81 contribution (GOLD cluster detection)
            let mod81_expected = pubkey_hexes.len() as f64 / 81.0;
            let mod81_deviation =
                (mod81_counts[mod81_residue] as f64 - mod81_expected) / mod81_expected;
    ...
    let gold_cluster: Vec<String> = sorted_scores
        .iter()
    ...
    let top_100: Vec<String> = sorted_scores
        .iter()
        .take(100)
    ...
    let high_bias: Vec<String> = sorted_scores
        .iter()
    ...
    // Statistics
    let avg_score =
        sorted_scores.iter().map(|(_, s, _)| s).sum::<f64>() / sorted_scores.len() as f64;
    let max_score = sorted_scores.first().map_or(0.0, |(_, s, _)| *s);
    ...
    println!("\n📊 Analysis Statistics:");
    println!("  📈 Average bias score: {avg_score:.3}x");
    println!("  🎯 Maximum bias score: {max_score:.3}x");
    println!(
        "  🏆 GOLD cluster addresses: {} ({:.2}%)",
        gold_cluster.len(),
        gold_percentage
    );
    println!(
    ...
    // Write results
    write_priority_list(
        &format!("{}_gold.txt", args.output.trim_end_matches(".txt")),
        &gold_cluster,
    )?;
    write_priority_list(
        &format!("{}_top100.txt", args.output.trim_end_matches(".txt")),
        &top_100,
    )?;
...
fn compute_modular_counts(pubkeys: &[String], modulus: u64) -> Vec<usize> {
    ...
    for pubkey_hex in pubkeys {
        ...
        if let Ok(bytes) = hex::decode(pubkey_hex) {
            if bytes.len() == 33 {
                if let Some(point) =
                    AffinePoint::from_bytes(bytes.as_slice().into()).into_option()
                {
                    let x_bytes = point.x();
                    let x_u64 = u64::from_be_bytes(x_bytes[..8].try_into().unwrap());
                    let x = BigInt256 {
...
fn write_priority_list(path: &str, pubkeys: &[String]) -> Result<()> {
    ...
    writeln!(
        file,
        "# Generated by speedbitcrack simple_bias_analyze tool"
    )?;
    writeln!(
        file,
        "# Statistical bias analysis for enhanced kangaroo efficiency"
    )?;
    writeln!(file, "# Total addresses: {}", pubkeys.len())?;
    writeln!(
        file,
        "# Analysis method: Real modular bias statistics (mod3/9/27/81)"
    )?;
    writeln!(
        file,
        "# GOLD cluster detection: mod81=0 pattern recognition"
    )?;
    writeln!(file)?;
