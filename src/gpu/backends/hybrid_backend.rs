...
use crate::types::{DpEntry, KangarooState, RhoState};
...
impl HybridBackend {
    ...
    pub fn cpu_staging_to_cuda(&self, staging: &CpuStagingBuffer) -> Result<()> {
                ...
                .any(|d| d.supports_unified_memory)
        {
            ...
            log::debug!(
                "Using optimized CPU→CUDA transfer via staging buffer (size: {} bytes)",
                staging.size
            );
        ...
        G: FnOnce(&[u8]) -> Result<Vec<u8>>,
    {
        ...
        log::info!(
            "Hybrid operation completed: Vulkan {}ms, Staging {}ms, CUDA {}ms, Total {}ms",
            vulkan_time,
            staging_time,
            cuda_time,
            total_time
        ...
        Err(anyhow!(
            "Hybrid operation result type conversion not yet specialized for this operation"
        ))
    }
    ...
    pub fn create_unified_buffer(&mut self, size: usize, label: &str) -> Result<()> {
        ...
        if self.zero_copy_enabled {
            #[cfg(feature = "wgpu")]
            {
                ...
                if external_memory_supported {
                    ...
                    log::info!(
                        "Created Vulkan buffer with external memory export for {}",
                        label
                    );
            ...
            if unified_buffer.zero_copy_enabled {
                ...
                log::info!(
                    "Imported Vulkan memory into CUDA for zero-copy access: {}",
                    label
                );
        ...
        if !unified_buffer.zero_copy_enabled {
            log::info!(
                "Zero-copy not available, buffer {} will use CPU staging",
                label
            );
        }

        self.unified_buffers
            .insert(label.to_string(), unified_buffer);
        Ok(())
    }
    ...
    pub fn unified_transfer(&self, buffer_name: &str, data: &[u8], offset: usize) -> Result<()> {
        if let Some(unified_buffer) = self.unified_buffers.get(buffer_name) {
            if unified_buffer.zero_copy_enabled {
                ...
                log::warn!(
                    "Zero-copy not available, using CPU staging for {}",
                    buffer_name
                );
    ...
    fn find_executable_work(&self, queue: &mut OooExecutionQueue) -> Option<WorkItem> {
        ...
        for work_item in candidates {
            ...
            let dependencies_satisfied = work_item
                .dependencies
                .iter()
                .all(|&dep_id| queue.completed_work.contains_key(&dep_id));
    ...
    fn execute_work_item(&self, work_item: WorkItem) -> tokio::task::JoinHandle<WorkResult> {
        let backend =
            self.select_backend_for_operation(&self.operation_to_string(&work_item.operation));
        ...
        tokio::spawn(async move {
            match work_item.operation {
                HybridOperation::BatchInverse(inputs, modulus) => {
                    match backend.as_str() {
                        "cuda" => {
                            ...
                            {
                                // Use CUDA backend
                                match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                    .batch_inverse(&inputs, modulus)
                                {
                        ...
                        "vulkan" => {
                            ...
                            {
                                // Use Vulkan backend
                                match crate::gpu::backends::vulkan_backend::WgpuBackend::new()
                                    .await
                                    .unwrap()
                                    .batch_inverse(&inputs, modulus)
                                {
                ...
                HybridOperation::BatchBarrettReduce(inputs, mu, modulus, use_montgomery) => {
                    match backend.as_str() {
                        "cuda" => {
                            ...
                            {
                                match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                    .batch_barrett_reduce(inputs, mu, modulus, use_montgomery)
                                {
                ...
                HybridOperation::BatchBigIntMul(a, b) => match backend.as_str() {
                    "cuda" => {
                        #[cfg(feature = "rustacuda")]
                        {
                            match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                .batch_bigint_mul(&a, &b)
                            {
                ...
                HybridOperation::StepBatch(mut positions, mut distances, types) => {
                    match backend.as_str() {
                        "cuda" => {
                            ...
                            {
                                match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                    .step_batch(&mut positions, &mut distances, &types)
                                {
                            ...
                            #[cfg(feature = "rustacuda")]
                            {
                                match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                    .batch_solve_collision(
                                        alpha_t, alpha_w, beta_t, beta_w, target, n,
                                    ) {
                    ...
                    match backend.as_str() {
                        "cuda" => {
                            #[cfg(feature = "rustacuda")]
                            {
                                ...
                                WorkResult::Error(anyhow!(
                                    "Custom CUDA operations not yet implemented: {}",
                                    operation_name
                                ))
                            }
                            #[cfg(not(feature = "rustacuda"))]
                        ...
                        "vulkan" => {
                            #[cfg(feature = "wgpu")]
                            {
                                ...
                                WorkResult::Error(anyhow!(
                                    "Custom Vulkan operations not yet implemented: {}",
                                    operation_name
                                ))
                            }
                            #[cfg(not(feature = "wgpu"))]
                        ...
                        _ => WorkResult::Error(anyhow!(
                            "Unsupported backend for custom operation: {}",
                            operation_name
                        )),
                    }
                }
                    ...
                    match backend.as_str() {
                        "cuda" => {
                            #[cfg(feature = "rustacuda")]
                            {
                                ...
                                match crate::gpu::backends::cuda_backend::CudaBackend::new()
                                    .batch_inverse(&vec![input], modulus)
                                {
                                    Ok(mut results) => {
    ...
    fn operation_to_string(&self, operation: &HybridOperation) -> String {
        match operation {
            ...
            HybridOperation::SolveCollision(_, _, _, _, _, _) => {
                "batch_solve_collision".to_string()
            }
    ...
    pub async fn execute_flow_pipeline(
        ...
        for stage in &pipeline.stages {
            ...
            self.update_bottleneck_analysis(
                &mut pipeline.performance_monitor,
                &stage.name,
                duration,
            );
    ...
    async fn execute_flow_stage(&self, stage: &FlowStage, _input_data: &[u8]) -> Result<Vec<u8>> {
        ...
        match &stage.operation {
            ...
            HybridOperation::BatchBarrettReduce(inputs, mu, modulus, use_montgomery) => {
                ...
                let results =
                    self.batch_barrett_reduce(inputs.clone(), *mu, *modulus, *use_montgomery)?;
                Ok(bincode::serialize(&results)?)
            }
            ...
            HybridOperation::SolveCollision(alpha_t, alpha_w, beta_t, beta_w, target, n) => {
                ...
                let results = self.batch_solve_collision(
                    alpha_t.clone(),
                    alpha_w.clone(),
                    beta_t.clone(),
                    beta_w.clone(),
            ...
            HybridOperation::DpCheck(_points, _mask) => {
                ...
                Err(anyhow!(
                    "DpCheck operation not implemented in flow pipeline"
                ))
            }
            HybridOperation::BsgsSolve(_points, _distances, _target) => {
                ...
                Err(anyhow!(
                    "BsgsSolve operation not implemented in flow pipeline"
                ))
            }
    ...
    fn update_bottleneck_analysis(
        &self,
        monitor: &mut PipelinePerformanceMonitor,
        stage_name: &str,
        ...
        if let Some(current_slowest) = &monitor.bottleneck_detection.slowest_stage {
            if let Some(current_latencies) = monitor.stage_latencies.get(current_slowest) {
                if let Some(avg_current) = current_latencies
                    .iter()
                    .sum::<std::time::Duration>()
                    .checked_div(current_latencies.len() as u32)
                {
        ...
        for (stage, latencies) in &monitor.stage_latencies {
            if !latencies.is_empty() {
                ...
                monitor
                    ...
                    .utilization_rates
                    .insert(stage.clone(), utilization);
            }
        }
    }
    ...
    fn generate_pipeline_optimizations(&self, pipeline: &mut FlowPipeline) {
            ...
            .slowest_stage
        {
            pipeline
                ...
                .optimization_suggestions
                .push(format!(
                    "Optimize {} stage - identified as bottleneck",
                    slowest_stage
                ));
        }

        // Analyze utilization imbalances
        let utilizations = &pipeline
            .performance_monitor
            .bottleneck_detection
            .utilization_rates;
        ...
        if !matches!(
            pipeline.work_distribution,
            WorkDistributionStrategy::Adaptive
        ) {
            pipeline.performance_monitor.optimization_suggestions.push(
                "Consider switching to adaptive work distribution for better performance"
                    .to_string(),
            );
        }
    }
    ...
    pub fn get_pipeline_performance(&self, pipeline: &FlowPipeline) -> PipelinePerformanceSummary {
        ...
        PipelinePerformanceSummary {
            ...
            optimization_suggestions: pipeline
                .performance_monitor
                .optimization_suggestions
                .clone(),
        }
    ...
    fn select_least_loaded_backend(
        &self,
        _operation: &str,
        context: &SchedulingContext,
    ) -> BackendSelection {
    ...
    fn select_best_performing_backend(
        &self,
        ...
        if let Some(history) = scheduler.backend_performance_history.get(operation) {
            ...
            let mut backend_performance: std::collections::HashMap<String, Vec<f64>> =
            ...
            let best_backend = backend_performance
                .iter()
    ...
    fn select_adaptive_backend(
        ...
        let vulkan_score = factors.vulkan_load * 0.3
            + factors.vulkan_performance * 0.4
            + factors.vulkan_memory * 0.3;
        let cuda_score =
            factors.cuda_load * 0.3 + factors.cuda_performance * 0.4 + factors.cuda_memory * 0.3;
        let cpu_score =
            factors.cpu_load * 0.3 + factors.cpu_performance * 0.4 + factors.cpu_memory * 0.3;
...
impl GpuCluster {
    ...
    pub fn new() -> Result<Self> {
        // For now, create a minimal cluster with placeholder devices
        // In production, this would enumerate actual GPU devices
        ...
        // Add secondary GPU if available (placeholder)
        devices.push(GpuDevice {
            id: 1,
        ...
        Ok(GpuCluster {
            devices,
            topology: GpuTopology {
                pci_bandwidth_matrix: vec![vec![100.0; 2]; 2], // 100 GB/s between devices
                numa_domains: vec![vec![0, 1]],                // Both devices in same NUMA domain
                nvlink_mask: vec![vec![false; 2]; 2],          // No NVLink (PCIe only)
            },
            power_management: PowerManagement {
                power_limit_per_gpu: 500.0, // Watts
                total_cluster_limit: 800.0, // Watts
                efficiency_optimizer: EfficiencyOptimizer {
    ...
    /// Distribute kangaroo herd across available GPUs
    pub fn distribute_kangaroos(
        ...
        // In production, this would consider load balancing, thermal constraints, etc.
        let base_allocation = total_kangaroos / self.devices.len();
        let remainder = total_kangaroos % self.devices.len();

        for (i, device) in self.devices.iter().enumerate() {
            let allocation = base_allocation + if i < remainder { 1 } else { 0 };
            distribution.insert(device.id, allocation);
        }

        distribution
    }

    /// Get optimal workgroup size for a device
    pub fn get_optimal_workgroup_size(&self, device_id: usize) -> u32 {
        // RTX 5090 optimal workgroup size
        256 // Could be tuned per device
    }

    /// Monitor cluster health and redistribute load if needed
    pub fn monitor_and_redistribute(&mut self) -> Result<()> {
        // Check temperatures and power consumption
        for device in &self.devices {
            if device.temperature > self.thermal_coordination.max_temp_per_gpu {
                log::warn!(
                    "Device {} temperature {}°C exceeds threshold {}°C",
                    device.id,
                    device.temperature,
        ...
        // In production, this would trigger load redistribution
        Ok(())
    }
}
...
impl HybridBackend {
    ...
    pub fn get_performance_summary(&self) -> HashMap<String, f64> {
        ...
        summary.insert(
            "vulkan_avg_time_ms".to_string(),
            if vulkan_ops > 0 {
                vulkan_time as f64 / vulkan_ops as f64
            } else {
                0.0
            },
        );
        summary.insert(
            "cuda_avg_time_ms".to_string(),
            if cuda_ops > 0 {
                cuda_time as f64 / cuda_ops as f64
            } else {
                0.0
            },
        );
...
impl HybridBackend {
        ...
        for i in 0..devices.len() {
            for j in 0..devices.len() {
                if i == j {
                    ...
                    // Same NUMA domain (4 GPUs per domain)
    ...
    pub fn profile_hashrates(config: &GpuConfig) -> (f64, f64) {
        ...
        let vulkan_fut = async {
            if !vulkan_batch.is_empty() {
                ...
                if !vulkan_positions.is_empty() && !vulkan_distances.is_empty() {
                    self.vulkan.step_batch_bias(
                        &mut vulkan_positions,
                        &mut vulkan_distances,
                        &vulkan_types,
                        config,
                    )?;
                }
        ...
        > = Box::pin(async {
            if !cuda_batch.is_empty() {
                ...
                let mut positions: Vec<[[u32; 8]; 3]> = cuda_batch
                    ...
                    .collect();
                let mut distances: Vec<[u32; 8]> = cuda_batch
                    .iter()
                    .map(|ks| ks.distance.to_u32_limbs())
                    .collect();
                let types: Vec<u32> = cuda_batch.iter().map(|ks| ks.kangaroo_type).collect();
    ...
    fn split_herd_for_hybrid<'a>(
        &self,
        herd: &'a [KangarooState],
    ) -> (&'a [KangarooState], &'a [KangarooState]) {
    ...
    pub fn adjust_gpu_frac(config: &mut GpuConfig, util: f64, temp: u32) {
    ...
    pub async fn dispatch_step_batch(
        ...
        if vulkan_ratio > 0.0 {
            ...
            if vulkan_end > vulkan_start {
                ...
                let mut vulkan_positions_vec: Vec<[[u32; 8]; 3]> =
                    positions[vulkan_start..vulkan_end].to_vec();
                let mut vulkan_distances_vec: Vec<[u32; 8]> =
                    distances[vulkan_start..vulkan_end].to_vec();
                let vulkan_types_vec: Vec<u32> = types[vulkan_start..vulkan_end].to_vec();

                match self.vulkan.step_batch(
                    &mut vulkan_positions_vec,
                    &mut vulkan_distances_vec,
                    &vulkan_types_vec,
                ) {
    ...
    pub fn optimize_based_on_metrics_production(
        ...
        if metrics.dram_utilization > 0.8 || metrics.l2_hit_rate < 0.7 {
            ...
            log::info!(
                "🎯 Memory optimization: kangaroos {} → {} (DRAM util {:.1}%, L2 hit {:.1}%)",
                old_count,
                config.max_kangaroos,
        ...
        if metrics.sm_efficiency < 0.7 || metrics.achieved_occupancy < 0.6 {
            ...
            log::info!(
                "🎯 Occupancy optimization: kangaroos {} → {} (occupancy {:.1}%, SM eff {:.1}%)",
                old_count,
                config.max_kangaroos,
        ...
        if metrics.alu_utilization > 0.9 && metrics.sm_efficiency > 0.8 {
            ...
            log::info!(
                "🎯 Compute optimization: kangaroos {} → {} (ALU util {:.1}%, SM eff {:.1}%)",
                old_count,
                config.max_kangaroos,
                metrics.alu_utilization * 100.0,
                metrics.sm_efficiency * 100.0
            );
            optimization_applied = true;
        }
        ...
        if metrics.register_usage > 64 {
            ...
            log::info!(
                "🎯 Register optimization: kangaroos {} → {} (register usage {}/255)",
                old_count,
                config.max_kangaroos,
                metrics.register_usage
            );
            optimization_applied = true;
        }
    ...
    pub fn apply_rule_based_adjustments(config: &mut GpuConfig) {
        ...
        if let Ok(json_str) = std::fs::read_to_string("suggestions.json") {
                ...
                serde_json::from_str::<std::collections::HashMap<String, String>>(&json_str)
            {
                ...
                if suggestions
                    .values()
                    .any(|s| s.contains("High Registers") || s.contains("reduce locals"))
                {
                ...
                if suggestions
                    .values()
                    .any(|s| s.contains("High Divergence") || s.contains("subgroup"))
                {
                    config.max_kangaroos = (config.max_kangaroos * 4 / 5).max(256);
                ...
                if suggestions
                    .values()
                    .any(|s| s.contains("modular") || s.contains("Barrett"))
                {
                    config.gpu_frac = (config.gpu_frac * 0.9).max(0.5);
                    adjustments_made
                        .push("Adjusted GPU fraction for modular arithmetic optimization");
                }
    ...
    pub fn dispatch_hybrid_scaled_with_rules_and_metrics(
        ...
        while completed < total_steps {
            ...
            if temp > 75 {
                config.max_kangaroos /= 2;
                log::warn!(
                    "Thermal throttling: reduced kangaroos to {} due to high temp ({}°C)",
                    config.max_kangaroos,
                    temp
                );
            }
    ...
    pub fn create_shared_buffer(&self, size: usize) -> anyhow::Result<SharedBuffer> {
        ...
        {
            Err(anyhow::anyhow!(
                "No GPU backends available for shared buffers"
            ))
        }
    }
}
...
impl GpuBackend for HybridBackend {
    ...
    fn batch_init_kangaroos(
        ...
        if self.cuda_available {
            return self
                .cuda
                .batch_init_kangaroos(tame_count, wild_count, targets);
        }
    ...
    fn precomp_table(
        &self,
        #[allow(unused_variables)] base: [[u32; 8]; 3],
        #[allow(unused_variables)] window: u32,
    ) -> Result<Vec<[[u32; 8]; 3]>> {
    ...
    fn step_batch(
        ...
        let result = match backend.as_str() {
            "vulkan" => {
                ...
                {
                    return Err(anyhow!(
                        "Vulkan backend not available for bulk stepping operations"
                    ));
                }
            }
            "cuda" => {
                ...
                {
                    return Err(anyhow!(
                        "CUDA backend not available for stepping operations"
                    ));
                }
            }
            _ => {
                return Err(anyhow!(
                    "No suitable GPU backend available for stepping operations"
                ));
            }
        };
        ...
        log::info!(
            "step_batch completed on {} backend in {}ms ({} kangaroos)",
            backend,
            duration,
            positions.len()
        );
    ...
    fn step_batch_bias(
        ...
        {
            self.vulkan
                .step_batch_bias(positions, distances, types, config)
        }
        #[cfg(not(feature = "wgpu"))]
        {
    ...
    fn batch_bsgs_solve(
        ...
        {
            self.cuda
                .batch_bsgs_solve(deltas, alphas, distances, config)
        }
        #[cfg(not(feature = "rustacuda"))]
        {
    ...
    fn batch_inverse(
        ...
        let result = match backend.as_str() {
            ...
            _ => self.cpu.batch_inverse(inputs, modulus),
        };
        ...
        log::info!(
            "batch_inverse completed on {} backend in {}ms ({} inputs)",
            backend,
            duration,
            inputs.len()
        );
    ...
    fn batch_solve_collision(
        ...
        {
            self.cuda
                .batch_solve_collision(alpha_t, alpha_w, beta_t, beta_w, target, n)
        }
        #[cfg(not(feature = "rustacuda"))]
        {
            self.cpu
    ...
    fn batch_barrett_reduce(
        ...
        {
            self.cuda
                .batch_barrett_reduce(x, mu, modulus, use_montgomery)
        }
        #[cfg(not(feature = "rustacuda"))]
        {
            self.cpu
    ...
    fn blend_proxy_preseed(
        ...
        {
            self.cuda
                .blend_proxy_preseed(preseed_pos, num_random, empirical_pos, weights)
        }
        #[cfg(not(feature = "rustacuda"))]
        {
            self.cpu
...
impl HybridBackend {
    ...
    fn parse_rule_suggestion(&self, _rule_name: &str, suggestion: &str) -> (f64, String) {
        ...
        let score = if let Some(pct_pos) = suggestion.find('%') {
            if let Some(start) =
                suggestion[..pct_pos].rfind(|c: char| !c.is_ascii_digit() && c != '.')
            {
        ...
        match rule.rule_name.as_str() {
            "Low Coalescing" => {
                if rule.score < 80.0 {
                    ...
                    log::info!(
                        "Reduced kangaroo count to {} for better coalescing",
                        config.max_kangaroos
                    );
                }
            }
            "High Registers" => {
                if rule.score < 70.0 {
                    ...
                    config.max_regs = 48;
                    log::info!(
                        "Limited registers to {} for better occupancy",
                        config.max_regs
                    );
                }
            }
            "DRAM Utilization" => {
                if rule.score > 80.0 {
                    ...
                    config.max_kangaroos = (config.max_kangaroos * 3 / 4).max(512);
                    log::info!(
                        "Reduced kangaroo count to {} due to high DRAM utilization",
                        config.max_kangaroos
                    );
                }
            }
    ...
    pub fn analyze_ecdlp_bias_efficiency(
        &self,
        metrics: &HashMap<String, f64>,
    ) -> NsightRuleResult {
        let alu_pct = metrics
            .get("sm__pipe_alu_cycles_active.average.pct_of_peak_sustained_active")
            .copied()
            .unwrap_or(0.0);
        let ipc = metrics
            .get("sm__inst_executed.avg.pct_of_peak_sustained_active")
            .copied()
            .unwrap_or(1.0);
    ...
    pub fn analyze_ecdlp_divergence(&self, metrics: &HashMap<String, f64>) -> NsightRuleResult {
        let warp_eff = metrics
            .get("sm__warps_active.avg.pct_of_peak_sustained_active")
            .copied()
            .unwrap_or(100.0);
        let branch_eff = metrics
            .get("sm__inst_executed.avg.pct_of_peak_sustained_elapsed")
            .copied()
            .unwrap_or(1.0);
    ...
    pub fn tune_ml_predict(&self, config: &mut GpuConfig) {
        ...
        log::info!(
            "ML prediction adjusted GPU fraction to {:.2}",
            config.gpu_frac
        );
    }
    ...
    #[allow(dead_code)]
    async fn check_and_resolve_collisions(
        &self,
        _dp_table: &crate::dp::DpTable,
        _states: &[RhoState],
    ) -> Option<BigInt256> {
    ...
    fn mul_glv_opt(&self, _p: [[u32; 8]; 3], _k: [u32; 8]) -> Result<[[u32; 8]; 3]> {
        ...
        #[cfg(feature = "wgpu")]
        if true {
            // Vulkan is available if feature is enabled
    ...
    fn mod_small(&self, _x: [u32; 8], _modulus: u32) -> Result<u32> {
        ...
        let modulus_extended = [
            modulus_bytes[0] as u32,
            modulus_bytes[1] as u32,
            modulus_bytes[2] as u32,
            modulus_bytes[3] as u32,
            0,
            0,
            0,
            0,
        ];
        let res = self.barrett_reduce(
            &x_extended.try_into().unwrap(),
            &modulus_extended,
            &compute_mu_small(_modulus),
        )?;
    ...
    fn batch_mod_small(&self, points: &Vec<[[u32; 8]; 3]>, modulus: u32) -> Result<Vec<u32>> {
        points
            .iter()
            .map(|p| self.mod_small(p[0], modulus))
            .collect()
    }
    ...
    pub fn convert_to_gpu_limbs(u64_arr: &[u64; 4]) -> [u32; 8] {
        ...
        for i in 0..4 {
            u32_arr[2 * i] = u64_arr[i] as u32;
            u32_arr[2 * i + 1] = (u64_arr[i] >> 32) as u32;
        }
        u32_arr
    }

    pub fn convert_from_gpu_limbs(u32_arr: &[u32; 8]) -> [u64; 4] {
        let mut u64_arr = [0u64; 4];
        for i in 0..4 {
            u64_arr[i] = (u32_arr[2 * i + 1] as u64) << 32 | u32_arr[2 * i] as u64;
        }
        u64_arr
    }
    ...
    fn generate_preseed_pos(
        ...
        Ok(crate::utils::bias::generate_preseed_pos(
            &min_scalar,
            &width_scalar,
        ))
    }
    ...
    fn blend_proxy_preseed(
        ...
        if true {
            // Vulkan is available if feature is enabled
            return self.vulkan.blend_proxy_preseed(
                preseed_pos,
                num_random,
                empirical_pos,
                weights,
        ...
        // Fallback to CPU implementation from utils::bias
        Ok(crate::utils::bias::blend_proxy_preseed(
            preseed_pos,
            num_random,
            empirical_pos,
            weights,
...
impl HybridBackend {
    ...
    pub fn glv_decompose_gpu(
        &self,
        scalars: &[crate::math::bigint::BigInt256],
    ) -> Result<
        Vec<(
            crate::math::bigint::BigInt256,
            crate::math::bigint::BigInt256,
        )>,
    > {
        #[cfg(feature = "rustacuda")]
        ...
        Ok(scalars.iter().map(|k| curve.glv_decompose(k)).collect())
    }

    /// CUDA GLV batch decomposition for massive scalar multiplication speedup
    #[cfg(feature = "rustacuda")]
    fn cuda_glv_decompose_batch(
        &self,
        _scalars: &[crate::math::bigint::BigInt256],
    ) -> Result<
        Vec<(
            crate::math::bigint::BigInt256,
            crate::math::bigint::BigInt256,
    ...
    > {
        ...
        Err(anyhow!(
            "CUDA GLV decomposition integration planned for Phase 5"
        ))
    }

    /// GPU-accelerated GLV4 decomposition for maximum speedup
    pub fn glv4_decompose_gpu(
        &self,
        scalars: &[k256::Scalar],
    ) -> Result<Vec<([k256::Scalar; 4], [i8; 4])>> {
        #[cfg(feature = "rustacuda")]
        if self.cuda_available {
