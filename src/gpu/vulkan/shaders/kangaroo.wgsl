...
@group(0) @binding(0) var<storage, read_write> states_x: array<u32, 4>;  // SoA: 4 limbs per kangaroo
@group(0) @binding(1) var<storage, read_write> states_y: array<u32, 4>;
@group(0) @binding(2) var<storage, read_write> states_dist: array<u32, 4>;
@group(0) @binding(3) var<storage, read_write> states_jump_idx: array<u32>;
@group(0) @binding(4) var<uniform> jumps: array<u32, 4>;  // Preloaded jump table (256-bit)
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Load SoA (coalesced)
    var px = states_x[idx * 4u .. (idx+1u)*4u];
    var py = states_y[idx * 4u .. (idx+1u)*4u];
    var dist = states_dist[idx * 4u .. (idx+1u)*4u];
    ...
    // Write back SoA
    states_x[idx * 4u .. (idx+1u)*4u] = px;
    states_y[idx * 4u .. (idx+1u)*4u] = py;
    states_dist[idx * 4u .. (idx+1u)*4u] = dist;
