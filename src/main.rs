...
//            }
    ...
    // Check if Puzzle #135 solving is requested
    if config.solve_puzzle_135 {
        println!("🎯 Solving Puzzle #135 - First Unsolved Challenge!");
        println!("💰 Reward: 0.135 BTC");
        println!("🎯 Target: REVEALED puzzle with known public key");

        let mut puzzle_config = config.clone();
...
fn run_real_puzzle_solving(config: &Config) -> Result<()> {
    ...
    if puzzle.status == speedbitcrack::puzzles::PuzzleStatus::Solved {
        // For solved puzzles, verify the known solution
        if let Some(ref expected_privkey) = puzzle.privkey_hex {
            println!("🔑 Known private key available - verifying solution...");
              ...
              let privkey = match BigInt256::manual_hex_to_bytes(expected_privkey) {
                  Ok(bytes) => {
                      if bytes.len() == 32 {
                          let mut arr = [0u8; 32];
                          arr.copy_from_slice(&bytes);
                          Some(BigInt256::from_bytes_be(&arr))
                      } else {
                          None
                      }
                  }
            ...
            match curve.mul_constant_time(&privkey, &curve.g) {
                Ok(computed_point) => {
                    let expected_bytes = hex::decode(&puzzle.pub_key_hex)?;
                    if expected_bytes.len() == 33 && (expected_bytes[0] == 0x02 || expected_bytes[0] == 0x03) {
                        let mut arr = [0u8; 33];
                        arr.copy_from_slice(&expected_bytes);
                        if let Some(expected_point) = curve.decompress_point(&arr) {
...
fn run_unsolved_kangaroo_algorithm(config: &Config, puzzle_num: u32) -> Result<()> {
    ...
    // Apply POP bias keyspace partitioning if enabled
    // This implements the user's histogram-based recursive keyspace reduction
    let search_ranges = if config.enable_pop_partitioning {
        println!("🎯 Applying POP bias keyspace partitioning...");
        speedbitcrack::utils::bias::pop_keyspace_partitioning(&target_point, get_puzzle_range(puzzle_num), puzzle_num)
    } else {
        vec![get_puzzle_range(puzzle_num)]
    };

    println!("🎯 Will search {} keyspace partition(s)", search_ranges.len());
    ...
    // Run the kangaroo algorithm on each partitioned range
    println!("🐪 Starting kangaroo algorithm with {} kangaroos, bias={:?}", gpu_config.max_kangaroos, bias);
    if config.enable_pop_partitioning {
        println!("🎯 POP partitioning active: Searching {} statistical partitions", search_ranges.len());
    }
    ...
    // Calculate partition size using hex representation for large ranges
    let original_range = get_puzzle_range(puzzle_num);
    let partition_size_hex = format!("0x{:x}", high.to_u64() - low.to_u64());
    let original_size_hex = format!("0x{:x}", original_range.1.to_u64() - original_range.0.to_u64());

    println!("   Partition size: {} (from {} to {})", partition_size_hex, low.to_hex(), high.to_hex());
...
fn get_puzzle_range(puzzle_num: u32) -> (BigInt256, BigInt256) {
    match puzzle_num {
        ...
        145 => (BigInt256::from_hex("1000000000000000000000000000000000000").unwrap(),
                BigInt256::from_hex("1ffffffffffffffffffffffffffffffffffff").unwrap()),
...
fn convert_valuable_p2pk_to_uncompressed() -> Result<()> {
    ...
    info!("🔄 Converting valuable_p2pk_pubkeys.txt to uncompressed format...");
    ...
    info!("📊 Processing {} valuable P2PK keys...", lines.len());
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        // Parse pubkey
        let result = match hex::decode(hex_str) {
            Ok(bytes) => {
                if bytes.len() == 65 && bytes[0] == 0x04 {
                    // Already uncompressed - validate and keep as-is
                    stats.uncompressed_already += 1;
                    validate_uncompressed_key(&bytes, i, hex_str)
                } else if bytes.len() == 33 && (bytes[0] == 0x02 || bytes[0] == 0x03) {
                    // Compressed - convert to uncompressed
                    stats.compressed_converted += 1;
                    convert_compressed_to_uncompressed(&bytes, i, hex_str)
                } else {
                    // Invalid format
        ...
        // Progress indicator
        if (i + 1) % 1000 == 0 {
            println!("📈 Processed {}/{} keys...", i + 1, lines.len());
        }
    }

    // Write output file (raw data only, no headers like input)
    let output_filename = "valuable_p2pk_uncompressed.txt";
    ...
    println!("├─ Total keys processed: {}", lines.len());
    println!("├─ Successfully converted: {}", converted_keys.len());
    ...
    info!("✅ Valuable P2PK conversion completed successfully");
    Ok(())
}

#[derive(Default)]
struct ConversionStats {
    uncompressed_already: usize,
    compressed_converted: usize,
...
fn validate_uncompressed_key(bytes: &[u8], _line_num: usize, hex_str: &str) -> Result<String> {
    ...
    // Create encoded point and validate
    let mut full_bytes = [0u8; 65];
    full_bytes[0] = 0x04;
    full_bytes[1..33].copy_from_slice(&x_bytes);
    full_bytes[33..65].copy_from_slice(&y_bytes);

    match k256::EncodedPoint::from_bytes(full_bytes) {
        Ok(encoded_point) => {
...
fn convert_compressed_to_uncompressed(bytes: &[u8], _line_num: usize, _hex_str: &str) -> Result<String> {
    ...
    match k256::EncodedPoint::from_bytes(bytes) {
        Ok(encoded_point) => {
            match k256::ProjectivePoint::from_encoded_point(&encoded_point) {
                projective_option if projective_option.is_some().into() => {
                    let projective_point = projective_option.unwrap();
                    let uncompressed_point = projective_point.to_encoded_point(false); // false = uncompressed

                    // Convert to hex string
                    let uncompressed_bytes = uncompressed_point.as_bytes();
                    let hex_result = hex::encode(uncompressed_bytes);
                    Ok(hex_result)
                }
        ...
        let point = match hex::decode(hex_str) {
            Ok(bytes) => {
                if bytes.len() == 65 && bytes[0] == 0x04 {
                    ...
                    match k256::EncodedPoint::from_bytes(bytes) {
                        Ok(encoded_point) => {
                            // from_encoded_point returns CtOption, not Result
                            let projective_point_option = k256::ProjectivePoint::from_encoded_point(&encoded_point);
                            if let Some(projective_point) = projective_point_option.into_option() {
                                // Convert to our Point format
                                Point::from_k256(&projective_point)
                            } else {
                                println!("⚠️  Failed to decompress compressed key at line {}: invalid point", i + 1);
                                continue;
                            }
                        }
                        Err(e) => {
                            println!("⚠️  Invalid compressed key format at line {}: {}", i + 1, e);
                            continue;
                        }
                    }
                } else {
