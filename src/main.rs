...
fn convert_valuable_p2pk_to_uncompressed() -> Result<()> {
    ...
    writeln!(output_file, "# Valuable P2PK Keys - All Converted to Uncompressed Format")?;
    writeln!(output_file, "# Generated by SpeedBitCrackV3 conversion utility")?;
    writeln!(output_file, "# Original file: valuable_p2pk_pubkeys.txt")?;
    writeln!(output_file, "# Total lines in original file: 34353")?;
    writeln!(output_file, "# Keys after filtering (non-empty, valid length): {}", lines.len())?;
    writeln!(output_file, "# Successfully converted: {}", converted_keys.len())?;
    writeln!(output_file, "# Invalid keys skipped: {}", invalid_keys.len())?;
    writeln!(output_file, "# Conversion statistics:")?;
    writeln!(output_file, "#   - Already uncompressed: {}", stats.uncompressed_already)?;
    writeln!(output_file, "#   - Compressed converted: {}", stats.compressed_converted)?;
    writeln!(output_file, "#   - Invalid format: {}", stats.invalid_format)?;
    writeln!(output_file, "#   - Invalid hex: {}", stats.invalid_hex)?;
    writeln!(output_file)?;

    // Write all successfully converted keys
    for key_hex in &converted_keys {
        writeln!(output_file, "{}", key_hex)?;
    }

    // Ensure we don't add a trailing newline
    output_file.flush()?;
