...
mod tests {
    ...
    use crate::math::bigint::{BarrettReducer, BigInt256, BigInt512};
    ...
    fn mod_pow_basic(base: &BigInt256, exp: &BigInt256, modulus: &BigInt256) -> BigInt256 {
        ...
        let mut b = barrett
            .reduce(&BigInt512::from_bigint256(&base.clone()))
            .unwrap();
        let mut e = exp.clone();
    ...
    fn test_tonelli_shanks_comprehensive() {
        ...
        assert!(
            root1.is_some(),
            "Tonelli-Shanks should find square root for quadratic residue"
        );

        let root1_val = root1.unwrap();
        // Verify: root^2 ≡ value mod modulus
        let root1_sq = curve.barrett_p.mul(&root1_val, &root1_val);
        let root1_sq_mod = curve
            .barrett_p
            .reduce(&BigInt512::from_bigint256(&root1_sq))
            .unwrap();
        assert!(
            root1_sq_mod == value1 || curve.barrett_p.sub(&modulus1, &root1_sq_mod) == value1,
        ...
        assert!(
            root2.is_some(),
            "Tonelli-Shanks should work with secp256k1 modulus"
        );

        let root2_val = root2.unwrap();
        let root2_sq = curve.barrett_p.mul(&root2_val, &root2_val);
        let root2_sq_mod = curve
            .barrett_p
            .reduce(&BigInt512::from_bigint256(&root2_sq))
            .unwrap();
        assert_eq!(
            root2_sq_mod, test_value,
            "Tonelli-Shanks should work for secp256k1"
        );
        ...
        assert!(
            root3.is_none(),
            "Tonelli-Shanks should return None for non-quadratic residues"
        );
...
fn check_lovasz(
    k: usize,
    mu: &[[BigInt256; DIM]; DIM],
    b_star: &[[BigInt256; DIM]; DIM],
    delta: &BigInt256,
) -> bool {
    let lhs = norm_squared(&b_star[k]);
    let rhs = (delta.clone() - mu[k][k - 1].clone() * mu[k][k - 1].clone())
        * norm_squared(&b_star[k - 1]);
    lhs >= rhs
}

#[test]
fn simulate_lll_proof() {
    let mut basis: [[BigInt256; DIM]; DIM] =
        core::array::from_fn(|_| core::array::from_fn(|_| BigInt256::zero()));
...
fn test_vow_with_babai() {
    ...
    if config.enable_vow_parallel {
        ...
        assert_eq!(
            result,
            crate::types::Scalar::new(crate::math::BigInt256::zero())
        );
    }
}
...
fn parallel_rho(
    #[allow(unused_variables)] pubkey: &k256::ProjectivePoint,
    #[allow(unused_variables)] m: usize,
) -> k256::Scalar {
