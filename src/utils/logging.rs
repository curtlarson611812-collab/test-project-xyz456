...
pub fn log_kangaroo_stats(total_kangaroos: usize, tame_count: usize, wild_count: usize) {
    info!(
        "Kangaroo stats: {} total ({} tame, {} wild)",
...
pub fn log_attractor_convergence(attractor_hash: &str, hit_count: usize, convergence_rate: f64) {
    info!(
        "Attractor convergence: {} hits on {}, rate {:.4}",
...
pub fn log_pruning_operation(entries_removed: usize, duration_ms: u64, method: &str) {
    info!(
        "DP pruning: removed {} entries in {}ms using {}",
...
pub fn log_collision_found(kangaroo_id: u64, distance: u64, total_ops: u64) {
    info!(
        "COLLISION DETECTED: kangaroo {}, distance {}, total ops {}",
...
pub fn log_checkpoint_summary(
    ...
    info!(
        "Checkpoint: {} ops in {:.1}s ({:.0} ops/sec), {} targets processed",
...
impl ProgressTracker {
    ...
    pub fn increment(&mut self, amount: u64) {
        ...
        if self.current % self.report_interval == 0 || self.last_report.elapsed().as_secs() >= 10 {
    ...
    fn report_progress(&self) {
        ...
        info!(
            "Progress: {}/{} ({:.1}%), {:.0}s elapsed, ETA {:.0}s",
            self.current,
            self.total,
            progress * 100.0,
            elapsed,
    ...
    pub fn complete(&self) {
        ...
        info!(
            "Completed: {} items in {:.2}s ({:.0} items/sec)",
...
pub fn load_comprehensive_nsight_metrics(path: &str) -> Option<NsightMetrics> {
    ...
    if let Some(warp_eff) = rho_kernel.get("occ_warp_execution_efficiency") {
        if let Some(s) = warp_eff.as_str() {
            metrics.warp_execution_efficiency =
                s.trim_end_matches('%').parse().unwrap_or(0.0) / 100.0;
        }
    }
    ...
    if let Some(alu_util) =
        rho_kernel.get("compute_sm__pipe_alu_cycles_active.average.pct_of_peak_sustained_active")
    {
    ...
    if let Some(inst_tp) =
        rho_kernel.get("compute_sm__inst_executed.avg.pct_of_peak_sustained_active")
    {
    ...
    if let Some(warp_eff) = rho_kernel.get("compute_warp_nonpred_execution_efficiency") {
        if let Some(s) = warp_eff.as_str() {
            metrics.warp_nonpred_efficiency =
                s.trim_end_matches('%').parse().unwrap_or(0.0) / 100.0;
        }
    }
    ...
    if let Some(recs) = all_metrics.get("optimization_recommendations") {
        if let Some(arr) = recs.as_array() {
            metrics.optimization_recommendations = arr
                .iter()
...
pub fn get_avg_temp(log_path: &str) -> Option<u32> {
    ...
    let temps: Vec<u32> = re
        .find_iter(&data)
        .filter_map(|m| m.as_str().trim_end_matches('C').parse().ok())
        .collect();
    if temps.len() > 10 {
...
pub fn generate_metric_based_recommendations(metrics: &NsightMetrics) -> Vec<String> {
    ...
    if metrics.sm_efficiency < 0.7 {
        recommendations.push(
            "Low SM efficiency (<70%) - reduce register usage or kernel unrolling".to_string(),
        );
    }

    if metrics.achieved_occupancy < 0.6 {
        recommendations
            .push("Low occupancy (<60%) - reduce block size or increase parallelism".to_string());
    }

    if metrics.warp_execution_efficiency < 0.9 {
        recommendations
            .push("Low warp execution efficiency (<90%) - reduce divergent branches".to_string());
    }

    // Memory recommendations
    if metrics.l2_hit_rate < 0.7 {
        recommendations.push(
            "Low L2 cache hit rate (<70%) - optimize data layout and access patterns".to_string(),
        );
    }

    if metrics.dram_utilization > 0.8 {
        recommendations.push(
            "High DRAM utilization (>80%) - memory bandwidth bound, improve coalescing".to_string(),
        );
    }
    ...
    if metrics.alu_utilization < 0.8 {
        recommendations.push(
            "Low ALU utilization (<80%) - fuse operations or reduce memory stalls".to_string(),
        );
    }

    if metrics.inst_throughput < 0.7 {
        recommendations
            .push("Low instruction throughput (<70%) - optimize kernel code".to_string());
    }

    if metrics.warp_nonpred_efficiency < 0.9 {
        recommendations.push(
            "Low warp non-predicate efficiency (<90%) - reduce control flow divergence".to_string(),
        );
    }
    ...
    if metrics.register_usage > 64 {
        recommendations.push(format!(
            "High register usage ({} > 64) - reduce local variables",
